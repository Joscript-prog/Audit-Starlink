...
// Stockage des photos en mémoire (Uint8Array)
const photoStore = {};

// Liste des EPI (avec leur description)
const EPI_LIST = [
...
async function handlePhotoUpload(e) {
  ...
  // Lire en ArrayBuffer puis convertir en Uint8Array (compatible docx ImageRun)
  const buf = await file.arrayBuffer();
  const u8 = new Uint8Array(buf);

  // Détecter le type d'image (jpg, png, gif, bmp...) — basé sur l'extension/MIME
  let imgType = "png";
  ...
  else if (mime.includes("webp")) imgType = "png"; // fallback : essayer png

  photoStore[key] = { data: u8, type: imgType, name: file.name };
...
// ============================================================
//  Helpers de récupération du formulaire
// ============================================================
function val(id) {
...
function resetForm() {
  ...
  document.querySelectorAll(".photo-preview").forEach(p => p.classList.remove("shown"));
...
async function generateDocument() {
  ...
  try {
    ...
    const COLOR_ACCENT = "2E75B6";        // Bleu clair
    const COLOR_HEADER_BG = "1F4E79";     // Bandeau titre principal
    const COLOR_TABLE_HEADER = "D5E8F0";  // Fond entêtes tableau
    const COLOR_LIGHT_BG = "EAF2F8";      // Fond zones photos vides
    ...
    // Paragraphe avec plusieurs cases à cocher (séparées par des espaces)
              ...
              new TableCell({
                ...
                children: [
                  new Paragraph({
                    ...
                    children: [new TextRun({ text: "RAPPORT D'AUDIT - INSTALLATION STARLINK", bold: true, size: 32, color: "FFFFFF", font: FONT })]
                  }),
                  new Paragraph({
                    alignment: AlignmentType.CENTER,
    ...
    // Tableau référence/auditeur/date
    ...
    // Titre de section type "1. Informations administratives du client"
    ...
    const tableInfosClient = new Table({
      ...
      rows: [
        new TableRow({
          children: [
            cell("Raison sociale du site audité", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("raison_sociale"), { width: 6240, columnSpan: 3 })
          ]
        }),
        new TableRow({
          children: [
            cell("Adresse", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("adresse"), { width: 6240, columnSpan: 3 })
          ]
        }),
        new TableRow({
          children: [
            cell("Code postal", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
        ...
        new TableRow({
          children: [
            cell("Horaire d'ouverture du site", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("horaire"), { width: 6240, columnSpan: 3 })
          ]
        }),
        new TableRow({
          children: [
            cell("Procédure d'accès", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("procedure_acces"), { width: 6240, columnSpan: 3 })
          ]
        }),
        new TableRow({
          children: [
            cell("Téléphone site", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell("Tél site : " + val("tel_site"), { width: 6240, columnSpan: 3 })
          ]
        }),
        new TableRow({
          children: [
            cell("Nom du contact client sur site", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("contact_nom"), { width: 6240, columnSpan: 3 })
          ]
        }),
        new TableRow({
          children: [
            cell("Fonction", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("contact_fonction"), { width: 6240, columnSpan: 3 })
          ]
        }),
        new TableRow({
          children: [
            cell("Téléphone / Mail contact", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell("Tél : " + val("contact_tel"), { width: 1560 }),
    ...
    // ========== SECTION 2.1 : Description emplacement ==========
    ...
    const tableEmpl = new Table({
      ...
      rows: [
        new TableRow({
          children: [
            cell("Type d'emplacement", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
        ...
        new TableRow({
          children: [
            cell("Description détaillée", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("empl_description"), { width: 6240 })
          ]
        }),
        new TableRow({
          children: [
            cell("Hauteur depuis le sol", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("empl_hauteur"), { width: 6240 })
          ]
        }),
        new TableRow({
          children: [
            cell("Accessibilité", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [
        ...
        new TableRow({
          children: [
            cell("Dégagement vers le ciel", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [
    ...
    const tableObstr = new Table({
      ...
      rows: [
        new TableRow({
          children: [
            cell("Résultat du test d'obstruction", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [
        ...
        new TableRow({
          children: [
            cell("Commentaire", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
    ...
    const tablePose = new Table({
      ...
      rows: [
        new TableRow({
          children: [
            cell("Type de support installé", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [
        ...
        new TableRow({
          children: [
            cell("Longueur de câble à prévoir", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell((val("cable_longueur") || "__") + " M  (câble fourni ≤25m / prolongateur 25-50m)", { width: 6240 })
          ]
        }),
        new TableRow({
          children: [
            cell("Type de cheminement câble", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [
        ...
        new TableRow({
          children: [
            cell("Point de pénétration", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [
        ...
        new TableRow({
          children: [
            cell("Étanchéité du passage", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [
        ...
        new TableRow({
          children: [
            cell("Hauteur maximale d'intervention", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
    ...
    const tableSynthese = new Table({
      ...
      rows: [
        new TableRow({
          children: [
            cell("Heure de début d'intervention", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("heure_debut"), { width: 6240 })
          ]
        }),
        new TableRow({
          children: [
            cell("Heure de fin d'intervention", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("heure_fin"), { width: 6240 })
          ]
        }),
        new TableRow({
          children: [
            cell("Durée totale à prévoir", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("duree_totale"), { width: 6240 })
          ]
        }),
        new TableRow({
          children: [
            cell("Nombre de techniciens", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            cell(val("nb_techniciens"), { width: 6240 })
          ]
        }),
        new TableRow({
          children: [
            cell("Nacelle à prévoir", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [ checkboxParagraph([
        ...
        new TableRow({
          children: [
            cell("Câblage supplémentaire", { width: 3120, shading: COLOR_TABLE_HEADER, textOpts: { bold: true } }),
            new TableCell({
              borders, width: { size: 6240, type: WidthType.DXA },
              margins: { top: 80, bottom: 80, left: 120, right: 120 },
              children: [ checkboxParagraph([
    ...
    // ========== Helper : Photo block ==========
    // Crée un cadre photo (avec image si présente, sinon TOUTE la cellule supprimée par retour null)
    function makePhotoBlock(label, photoKey, opts = {}) {
      const photo = photoStore[photoKey];
      if (!photo) return null;  // ⇒ supprimer le placeholder

      // Cadre simple : titre + image
      const targetW = opts.width || 240;  // px
      const targetH = opts.height || 180;
    ...
    // Met deux blocs photo en ligne (table à 2 colonnes), ou 1 seul si l'autre est manquant
    function makePhotoRow(p1Label, p1Key, p2Label, p2Key) {
      const has1 = !!photoStore[p1Key];
      const has2 = !!photoStore[p2Key];
      if (!has1 && !has2) return [];   // rien : on saute la ligne complète

      // Si une seule photo : on l'affiche pleine largeur, taille un peu plus grande
      if (has1 !== has2) {
        const key = has1 ? p1Key : p2Key;
        const lab = has1 ? p1Label : p2Label;
        return [
          makePhotoBlock(lab, key, { tableWidth: 9360, width: 380, height: 260 }),
          emptyP()
        ];
      }

      // Les deux photos : on utilise une table à 2 colonnes pour les aligner
      return [
        new Table({
          width: { size: 9360, type: WidthType.DXA },
          columnWidths: [4680, 4680],
    ...
    // Crée une ligne unique (1 photo seule pleine largeur, ou rien)
    function makeSinglePhoto(label, key) {
      if (!photoStore[key]) return [];
      return [
        makePhotoBlock(label, key, { tableWidth: 9360, width: 400, height: 260 }),
        emptyP()
      ];
    }
    ...
    // SECTION 6 - Synthèse (numéroté "4" comme dans l'original)
